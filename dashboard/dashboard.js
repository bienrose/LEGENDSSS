const map = L.map('map').setView([14.5764, 121.0851], 15);
  ...
  document.getElementById('done-btn').addEventListener('click', () => filterPanel.classList.remove('open'));
  ...
  map.on('click', async function(e) {
    ...
    try {
      ...
      console.log('[SPOT] Location clicked:', {
        lat: currentClickLat,
        lng: currentClickLng,
  ...
  document.getElementById('closeSV').onclick = () => {
    const svDiv = document.getElementById('street-view');
    svDiv.style.display = 'none'; svDiv.innerHTML = '';
  ...
  function promptUnsaveLocation(id) {
    ...
    if (!loc) return;
    document.getElementById('unsave-msg').textContent =
      `Remove "${loc.locationName}" from saved?`;
  ...
  document.getElementById('cancel-logout').addEventListener('click',  () => logoutModal.classList.remove('open'));
